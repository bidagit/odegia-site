...
export function Levels() {
  return (
    <section
      id="niveaux"
      className="scroll-mt-20 bg-petrol-deep py-20 text-sand md:py-28"
    >
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        <div className="max-w-2xl">
          <span className="font-mono text-[11px] uppercase tracking-[0.16em] text-sand/50">
            L&apos;échelle
          </span>
          <h2 className="mt-3 text-[30px] font-semibold leading-[1.08] tracking-[-0.03em] md:text-[40px]">
            Six niveaux, trois qui se livrent.
          </h2>
          <p className="mt-4 text-[14.5px] leading-[1.7] text-sand/65">
          ...
          {LEVELS.map((l) => (
            <li
              ...
              className={`rounded-[20px] border p-6 ${
                l.target
                  ? "border-ocre bg-ocre/12"
                  : l.sellable
                    ? "border-sand/25 bg-sand/[0.07]"
                    : "border-sand/12 bg-sand/[0.03]"
              }`}
            >
              ...
              <p
                className={`mt-3 text-[13px] leading-[1.65] ${
                  l.sellable ? "text-sand/70" : "text-sand/45"
                }`}
              >
                {l.body}
              </p>
              {l.sellable && (
                <p
                  className={`mt-4 inline-block rounded-full px-3 py-1 text-[11px] font-medium uppercase tracking-[0.06em] ${
                    l.target
                      ? "bg-ocre text-ink"
                      : "border border-sand/30 text-sand/75"
                  }`}
                >
                  {l.sellable}
                </p>
        ...
        <div className="mt-10 rounded-[22px] border border-sand/15 bg-sand/[0.05] p-6 md:p-8">
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_1fr] lg:gap-14">
            <div>
              ...
              <p className="mt-3 text-[13.5px] leading-[1.7] text-sand/65">
                {LEVEL_RULE.body}
              </p>
            </div>
            <dl className="divide-y divide-sand/15 border-t border-sand/15">
              {LEVEL_RULE.rows.map((r) => (
                <div
                  key={r.when}
                  className="flex flex-col gap-1 py-3.5 sm:flex-row sm:items-baseline sm:justify-between sm:gap-6"
                >
                  <dt className="text-[13px] text-sand/60">{r.when}</dt>
