...
export function Hero() {
  return (
    <section className="relative overflow-hidden">
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute left-1/2 top-0 h-[560px] w-[900px] -translate-x-1/2 rounded-full bg-petrol/8 blur-3xl"
      />
        ...
        <div className="grid grid-cols-1 items-center gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)_auto] lg:gap-8">
          ...
          <div className="lg:pb-10">
            <span className="inline-flex items-center gap-2.5 rounded-full border border-ink/12 bg-surface/70 px-3.5 py-1.5 text-[12px] font-medium text-ink-soft">
              <span aria-hidden className="h-2 w-2 rounded-full bg-ocre" />
              {HERO.eyebrow}
            </span>

            <h1 className="mt-6 text-[38px] font-semibold leading-[0.98] tracking-[-0.035em] sm:text-[46px] lg:text-[54px] xl:text-[62px]">
              {HERO.title[0]}
              <br />
              <span className="text-petrol">{HERO.title[1]}</span>
            </h1>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-3">
              <a
                ...
                className="group inline-flex items-center gap-2 rounded-full bg-petrol px-6 py-3.5 text-[14.5px] font-medium text-white transition-colors hover:bg-petrol-deep"
              >
              ...
              <Link
                ...
                className="rounded-full border border-ink/15 px-6 py-3.5 text-[14.5px] font-medium transition-colors hover:border-petrol hover:text-petrol"
              >
          ...
          <div className="relative">
            <div className="relative aspect-square overflow-hidden rounded-[28px] bg-sand-deep shadow-[0_28px_60px_-28px_rgba(9,50,55,0.45)]">
              <HeroMedia />
            </div>
          </div>
            ...
            <ul className="flex flex-row justify-between gap-6 lg:flex-col lg:justify-start lg:gap-8">
              {HERO.stats.map((s) => (
                <li key={s.label} className="lg:border-t lg:border-ink/12 lg:pt-5">
                  <div className="text-[26px] font-semibold leading-none tracking-[-0.03em] text-petrol lg:text-[30px]">
                    {s.value}
                  </div>
